...
async function fetchBusinessWithLocation() {
    document.getElementById("mainHeading").innerHTML = "Welcome to " + localStorage.getItem("location").capitalize();
    ...
    if (response.status == 200) { //success
        ...
        responseData.forEach(element => {
            $('#displayShopID').append(`<div id=${element._id} class="card menu-container">
          ...
          <div class="vertical-center">
            ...
            <h5 class="card-text"><a href="https://${element.businessURL}/" target="_blank">${element.businessURL}</a></h5>
            </div>
            <div class="card-footer text-center">
                <a href="#" class="btn btn-primary" onclick="saveBusinessInLocal('${element.businessEmail}', '${element.businessName.replace(/'/g, "\\'")}');">VIEW MENU</a>
            </div>
          </div>
        </div>`);
            /* <div class="card" style="width: 18rem;">
              <div class="card-body">
                ...
                <p class="menu-p card-text">${element.businessAddress}</p>
                <p class="menu-p card-text">Contact: ${element.businessPhone}</p>
                <p class="menu-p card-text"><a href=${element.businessURL} target="_blank">${element.businessURL}</a></p>
                <a href="#" class="btn btn-primary">Go somewhere</a>
              </div>
            </div> */

            /*  $('#displayShopID').append(`<div id=${element._id}>
          <div class="menu-container text-left">
              ...
              <p class="menu-p"><a href="https://${element.businessURL}/" target="_blank">${element.businessURL}</a></p>
              <button style="margin-left: 100px;" type="button" class="btn btn-info btn-lg " 
              onclick="saveBusinessInLocal('${element.businessEmail}', '${element.businessName.replace(/'/g, "\\'")}');">VIEW MENU</button>
              
          </div>
      </div>`); */
        });
        document.getElementById("custNameId").innerHTML = "Hi, " + localStorage.getItem("customerName");
...
async function fetchBusinessMenus() {
    document.getElementById("restaurant").innerHTML = decodeURI(localStorage.getItem("tempBusinessName"));
...
async function checkCompleteAndUpdate() {
    if (orderOject.id > 0 && document.getElementById("orderAddress").value.length >= 7) {
        orderOject.isReserved = true;
        document.getElementById("warningMessage").classList.remove("warning");
        document.getElementById("warningMessage").innerHTML = "";
        const apiPath = 'https://cs615-project.herokuapp.com/delivery/setDeliveryReservedStatus';
        const response = await fetch(apiPath, {
                method: "PUT",
        ...
        if (orderOject.id <= 0 && document.getElementById("orderAddress").value.length < 7) {
            document.getElementById("warningMessage").innerHTML = "Time Slot and Address is missing!!!"
        } else if (orderOject.id <= 0) {
            document.getElementById("warningMessage").innerHTML = "Select Order Time Slot!!!"
        } else if (document.getElementById("orderAddress").value.length < 7) {
            document.getElementById("warningMessage").innerHTML = "Address should be less atleast 7 characters!!!"
        }
    }
}
...
async function fetchBusinessDelivery() {
    const apiUrl = 'https://cs615-proj.herokuapp.com/restaurant/getCurrentDayDeliveryTime';
    let _data = {
        businessEmail: localStorage.getItem("tempBusinessEmail"),
        dayName: weekday[new Date().getDay()]
    }
    ...
    if (response.status == 200) { //success
        ...
        responseData.datTime.forEach(element => {
            ...
            <input type="checkbox" class="form-check-input ${element.isReserved?'':'enableOrDisable'}" id="${element.id}" ${element.isReserved?'checked':''} ${element.isReserved?'disabled':''} onclick="updateDeliveryTime(this);">
        </div></div>
            </div>
            <hr>`);
        });
